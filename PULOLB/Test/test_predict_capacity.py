...
charge_partial, discharge_partial = predict_capacity.sort_data.charge_discharge(filename_partial)
charge_full, discharge_full = predict_capacity.sort_data.charge_discharge(filename_full)
...
def test_distance_cycle_to_full():
    df = predict_capacity.distance_cycle_to_full(1,PL03d_short,full_curvesd_short)
    ...
    if len(df) != len(full_curvesd_short.keys()):
        raise Exception('The number of the keys in the full cycle dict should match with the length of the returned dataframe' )
    else:
        pass

def test_partial_to_full():
    D,C,F = predict_capacity.partial_to_full(PL03d_short,full_curvesd_short)
    #Make sure the number of partial discharge keys are the same with the list length
    if len(C) == len(PL03d_short.keys()):
        pass
    else:
        raise Exception('Number of keys in partial dict is unmatched with the list length')

    #Make sure the length of distance dict and cycle dict are the same
    if len(D) == len(F):
        pass
    else:
        raise Exception('The length of distance dict and cycle dict dont match')
def test_get_lifetime():
    Percent, Time, Slope, Intercept, Life = predict_capacity.get_lifetime(PL03d_short,full_curvesd_short,1.5)
    #We are not checking the full cycle dictionary because it is built from our files and the data structure is known and well defined.

    #User input dict check
    #Check the input data type
    if (type(PL03d_short) == dict) & (type(full_curvesd_short) == dict):
        pass
    else:
    ...
    for i in PL03d_short.keys():
        if 'time' in PL03d_short[i].columns:
            ...
            raise Exception('There should be a time(lowercase) column in the dataframe')

    for i in PL03d_short.keys():
        if 'voltage' in PL03d_short[i].columns:
            pass
        else:
            raise Exception('There should be a voltage(lowercase) column in the dataframe')
    ...
    #Skip the first full cycle
        if i == 1:
            ...
            for x in PL03d_short[i]['voltage']:
                if x > 4:
                    raise ValueError('The partial discharge voltages should not be greater than 4V')
                else:
                    pass
    #Check the percentage, slope, and intercept
    for i in Percent:
        if i > 100:
            raise ValueError('The percentage of a battery should not exceed 100%')
        else:
            pass
    for i in Slope:
        if i > 0:
